...
1. **특성 중요도 분석 가능능**: 어떤 요소가 매칭 공정성에 가장 큰 영향을 미치는지 정량적으로 측정 가능
2. **비선형 관계 포착**: 복잡한 팀 구성과 실력 분산 간의 상호작용 분석
3. **모드별 비교 가능**: 동일한 모델로 모든 모드 분석이 가능해 공정한 비교
4. **Overfitting 방지**: 여러 의사결정나무의 투표로 공정한 결과
